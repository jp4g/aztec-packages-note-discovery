...
# Extract genesis values from yarn-project
...
# Compute GENESIS_ARCHIVE_ROOT based on SPONSORED_FPC and TEST_ACCOUNTS settings
# This matches the logic in yarn-project/cli/src/cmds/l1/deploy_l1_contracts_cmd.ts
# Defaults match those in spartan/scripts/deploy_network.sh
_SPONSORED_FPC="${SPONSORED_FPC:-true}"
_TEST_ACCOUNTS="${TEST_ACCOUNTS:-false}"
log "Computing genesis with SPONSORED_FPC=$_SPONSORED_FPC, TEST_ACCOUNTS=$_TEST_ACCOUNTS"
export GENESIS_ARCHIVE_ROOT=$(SPONSORED_FPC="$_SPONSORED_FPC" TEST_ACCOUNTS="$_TEST_ACCOUNTS" node --input-type=module -e "
...
const testAccounts = process.env.TEST_ACCOUNTS === 'true';
const sponsoredFPC = process.env.SPONSORED_FPC === 'true';

let initialAccounts = [];
if (testAccounts) {
  const data = await getInitialTestAccountsData();
  initialAccounts = data.map(a => a.address);
}

let sponsoredFPCAddresses = [];
if (sponsoredFPC) {
  const { SponsoredFPCContract } = await import('@aztec/noir-contracts.js/SponsoredFPC');
  const instance = await getContractInstanceFromInstantiationParams(SponsoredFPCContract.artifact, {
    salt: new Fr(SPONSORED_FPC_SALT),
  });
  sponsoredFPCAddresses = [instance.address];
}

const initialFundedAccounts = initialAccounts.concat(sponsoredFPCAddresses);
