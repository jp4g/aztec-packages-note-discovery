...
- Aztec Governance can vote to deploy a new CoinIssuer contract with a different rate
...
Slashing proposals are submitted to L1 for execution via the Slasher contract. When a sequencer's balance is slashed below the `EJECTION_THRESHOLD` (50e18 tokens, half the activation threshold of 100e18), they are removed from the sequencer set.
