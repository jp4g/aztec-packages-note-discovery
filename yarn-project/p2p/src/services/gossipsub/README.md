...
3. Timer starts for `meshMessageDeliveriesWindow` duration (2 seconds)
...
However, real-world implementations use larger values due to practical constraints:
- **Prysm** (Go): 2 seconds - the go-libp2p default
- **Lodestar** (TypeScript): 12 seconds - due to JavaScript I/O lag
- **Ethereum spec proposal**: 400ms was proposed but rejected as "too tight"

We use **2 seconds** (matching Prysm/go-libp2p default) as a balance between:
- Spec compliance: Small enough to limit replay-based score inflation
- Practical tolerance: Large enough to handle normal network propagation delays
- Precedent: Matches the most widely-used production value (Prysm)
...
This matches Ethereum's approach: **honest peers with temporary issues are inconvenienced but not punished**.x§
